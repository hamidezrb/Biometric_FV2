...
from q2 import calculate_q2_from_mask
from q3 import calculate_q3
from q5 import calculate_q5
from q5 import calculate_q5_ISO
from q7 import calculate_q7
from q7 import calculate_q7_ISO
from q9 import compute_q9_feature_points
...
def run_on_image(image_path: str, out_dir: str):
    ...
    Q1, Sunocc, R_mask, gray = calculate_q1(image_path)
    Q2, cx, cy, S_H, S_V = calculate_q2_from_mask(R_mask, gray)

    # --- Q3 + Q4 + Q6 + (your Q8)
    Q3, Q4, Q6, Q8, sigma, g_mean, N100, N_vessel = calculate_q3(R_mask, gray, Sunocc)

    # --- Q5
    # Q5, H_bits = calculate_q5(R_mask, gray)
    Q5, H_bits = calculate_q5_ISO(R_mask, gray, bit_depth=8, ep_c=0.75)

    # --- Q7
    # Q7, block_var = calculate_q7(R_mask, gray, g_mean)
    Q7, block_var = calculate_q7_ISO(R_mask, gray, g_mean)
    ...
    # --- Q9
    Q9, Ne, Ni = compute_q9_feature_points(vessel_binary, fg_bool, Cf=15)
    ...
    skel, endpoints, intersections = extract_points_from_q9(vessel_binary, fg_bool)
...
def main():
    ...
    for p in paths:
        try:
            ...
            print(
                f"{r['file']}: "
                f"Q1={r['Q1']} Q2={r['Q2']} Q3={r['Q3']} Q4={r['Q4']} Q5={r['Q5']} "
                f"Q6={r['Q6']} Q7={r['Q7']} Q8={r['Q8']} Q9={r['Q9']} "
                f"(Ne={r['Ne']}, Ni={r['Ni']})"
            )
    ...
    keys = ["file","Q1","Sunocc","Q2","Q3","sigma","g_mean","Q4","Q5","H_bits","Q6","N100","Q7","Q8","N_vessel","Q9","Ne","Ni"]
